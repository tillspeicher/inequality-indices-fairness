...
def upload_results(src_files, src_files_prefix, svr_project_path, file_extension_filter=None):
    ...
    NUM_TRIES = 3
    print('Uploading results to {}...'.format(upload_config['ssh-addr']))
...
def lookup_upload_config():
    ...
    config = {}
    for config_property in ['ssh-addr', 'web-addr', 'user']:
...
def web_attachment(url_postfix):
    upload_config = lookup_upload_config()
    base_url = 'https://{}/~{}/results/'.format(
            upload_config['web-addr'],
            upload_config['user'])
    return '{{' + base_url + url_postfix  + '||width=1000}}'
